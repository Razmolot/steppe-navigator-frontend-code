...
export const RiasecTestPage = () => {
  ...
  const initSession = async () => {
    try {
      ...
      const { data: sessionData } = await axiosClient.post("/student/riasec/sessions", {
        assignment_id: id,
        locale: locale,
      });
      ...
      if (error?.response?.status === 409 && error?.response?.data?.status === 'completed') {
        ...
        navigate({ to: `/student/tests/riasec/result/${sessionId}` });
        return;
      }
  ...
  const loadCurrentCase = async (sessionId: string) => {
    try {
      const { data } = await axiosClient.get(`/student/riasec/sessions/${sessionId}/cases/next`);
  ...
  const handleSaveAndNavigate = async (direction: 'next' | 'previous') => {
    ...
    if (direction === 'next' && answer.length === currentCase.options.length) {
      try {
        await axiosClient.post(`/student/riasec/sessions/${session.id}/answers`, {
        ...
        if (error?.response?.status === 409 || 
            ...
            (error?.response?.status === 400 && error?.response?.data?.status === 'completed')) {
          message.success(t.tests.testCompleted);
          navigate({ to: `/student/tests/riasec/result/${session.id}` });
        } else {
    ...
    try {
      let endpoint = '';
      if (direction === 'next') {
        endpoint = `/student/riasec/sessions/${session.id}/cases/next?current_case_id=${currentCase.case_id}&current_block_id=${currentCase.block_id}`;
      } else {
        endpoint = `/student/riasec/sessions/${session.id}/cases/previous?current_case_id=${currentCase.case_id}&current_block_id=${currentCase.block_id}`;
      }
  ...
  const questionOptions = useMemo(() => {
    ...
    if (answer.length === currentCase.options.length) {
      ...
      }).filter(Boolean) as Array<{ option_id: string; label: string; riasec: string; value: string }>;
    }
  ...
  return (
    <div className="counselor-tests-page" ref={topRef}>
      ...
      <div className="px-6 pb-6">
        ...
        <div className="ant-card ant-card-bordered riasec-content">
          <div className="ant-card-body">
            {/* Заголовок блока */}
            <h1 className="riasec-title">{getLocalizedText(currentCase.block_title)}</h1>
            ...
            {currentCase.block_description && (
              <div className={`riasec-block-description ${isDescriptionExpanded ? 'expanded' : 'collapsed'}`}>
                <p className="riasec-block-description-text">{getLocalizedText(currentCase.block_description)}</p>
                <button 
                  className="riasec-block-description-toggle"
            ...
            {/* Подзаголовок (вопрос) */}
            <h2 className="riasec-subtitle">{getLocalizedText(currentCase.stem)}</h2>

            {/* Инструкция */}
            <div className="riasec-instruction">
